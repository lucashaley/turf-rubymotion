...
- (void)setStartAndEndForPathNum:(int)pathNum
{
    ...
    double closestStartDist = INFINITY;
    double closestEndDist   = INFINITY;
    ...
    for (NSValue *key in [vertices allKeys]) {
        ...
        if (![self boundingBoxSharesEdgeWithVertex:v]) {
            ...
            double stDist = [self distanceFromPoint:startPoint toVertex:v];
            double edDist = [self distanceFromPoint:endPoint   toVertex:v];
...
{
    double incomingX = [v x];
    double incomingY = [v y];
    
    for (NSValue *key in [vertices allKeys]) {
        Vertex *existing = [vertices objectForKey:key];
        double existingX = [existing x];
        double existingY = [existing y];
...
{
    double boundsMinX = theBounds.origin.x;
    double boundsMaxX = theBounds.origin.x + theBounds.size.width;
    double boundsMinY = theBounds.origin.y;
    double boundsMaxY = theBounds.origin.y + theBounds.size.height;
